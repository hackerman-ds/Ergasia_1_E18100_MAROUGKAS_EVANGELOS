...
def create_user():
    ...
    if not users.find({"username":data["username"]}).count() > 0:
        ...
        return(Response(data["username"]+ " was not added to the database. It already exists", status=400, mimetype="application/json"))
...
def get_student():
    ...
    if not "email" in data:
        return Response("Information incomplete",status=500,mimetype="application/json")
    if is_session_valid(request.headers["uuid"]):
        student = students.find_one({"email":data["email"]})

        if not student == None:
            return Response(json.dumps({"name":student["name"], "email":student["email"], "yearOfBirth":student["yearOfBirth"], "address":student["address"]}), status=200, mimetype='application/json')
        else:
            return Response("No results have been found", status=400, mimetype="application/json")


    else:
        return Response('USER NOT VERIFIED', status=401, mimetype='application/json')
...
def get_students_thirty1():
    ...
    if is_session_valid(request.headers["uuid"]):
        student_thirties = students.find({"yearOfBirth":1991})
 
        student = []
        c = 0
        try:
            while True:
                student.append(student_thirties[c]["name"])
                c += 1
        except IndexError:
...
def get_students_thirty2():
    if is_session_valid(request.headers["uuid"]): 
        student_thirties = students.find({"yearOfBirth": {"$lt": 1991}})
...
def get_student2():
    ...
    if is_session_valid(request.headers["uuid"]):
        ...
        if not student == None:
            return Response(json.dumps({ "email":student["email"], "address":student["address"]}), status=200, mimetype='application/json')
        else:
            return Response("No results have been found", status=400, mimetype="application/json")
...
def delete_student():
    ...
    if  is_session_valid(request.headers["uuid"]):
        ...
        if not student == None:
            ...
            msg = "The student " + student["name"] + " was deleted"
...
def add_courses():
    ...
    if   is_session_valid(request.headers["uuid"]):
        ...
        if not student == None:
            ...
            return Response("THERE IS NO STUDENT WITH THAT EMAIL TRY AGAIN LATER", status=400, mimetype='application/json')
...
def get_courses():
    ...
    if   is_session_valid(request.headers["uuid"]):
            ...
            if not student == None:
                ...
                return Response("No student with that email", status=400, mimetype='application/json')
